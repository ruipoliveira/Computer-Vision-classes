...
int main(int argc, char **argv)
{
  ...
  //cenas adicionais
    ...
    std::cout << std::endl << rvecs.at(i);
      ...
      //ex1 object points para imagem
      std::vector<std::vector<cv::Point3f> > meus_object_points;
      std::vector<std::vector<cv::Point2f> > meus_image_points;

      std::vector<cv::Point3f> obj2;
      for(int j=0;j<board_sz;j++)
          obj2.push_back(cv::Point3f(float(j/board_w), float(j%board_w), 0.0));
    //      obj2.push_back(cv::Point3f(float(j/board_w), float(j%board_w), 0.0));

      projectPoints(object_points, rvecs, tvecs, intrinsic, distCoeffs, meus_image_points);
